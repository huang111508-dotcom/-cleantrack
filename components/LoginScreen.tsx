    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center items-center p-4 animate-fade-in relative">
      <div className="absolute top-4 left-4 z-40">
         <button 
           ...
           className="flex items-center gap-2 px-4 py-2 bg-white rounded-full shadow-sm border border-slate-200 text-sm font-bold text-slate-600 hover:text-brand-600 hover:border-brand-200 transition-all"
           title={language === 'zh' ? '多设备同步设置' : 'Sync Settings'}
         >
           <Settings size={16} />
           <span>{language === 'zh' ? '配置云端同步' : 'Setup Cloud Sync'}</span>
         </button>
      </div>
